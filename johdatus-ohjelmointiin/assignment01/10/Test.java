...
void main() throws Exception {
    ...
    if (lines.size() >= 2) {
        ...
        if (ascending) {
            IO.println("  PASS: numbers are in ascending order");
            passed++;
        } else {
            IO.println("  FAIL: numbers should be in ascending order");
        }
    } else {
    ...
    total++;
    if (!lines.isEmpty() && lines.get(0).equals("1")) {
        IO.println("  PASS: first number is 1");
        passed++;
    } else {
        var actual = lines.isEmpty() ? "(empty)" : lines.get(0);
        IO.println("  FAIL: first number should be 1, got: " + actual);
    }

    total++;
    if (!lines.isEmpty() && lines.get(lines.size() - 1).equals("10")) {
        IO.println("  PASS: last number is 10");
        passed++;
    } else {
        var actual = lines.isEmpty() ? "(empty)" : lines.get(lines.size() - 1);
        IO.println("  FAIL: last number should be 10, got: " + actual);
    }
