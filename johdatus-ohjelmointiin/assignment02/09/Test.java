...
void main() throws Exception {
    ...
    // Part d: every second number from 10 to 0
    total++;
    if (lines.size() >= 38) {
    ...
    // Check while-vs-dowhile.txt exists
    total++;
    if (Files.exists(Path.of("while-vs-dowhile.txt"))) {
        IO.println("  PASS: file while-vs-dowhile.txt exists");
        passed++;
    } else {
        IO.println("  FAIL: file while-vs-dowhile.txt not found");
    }
