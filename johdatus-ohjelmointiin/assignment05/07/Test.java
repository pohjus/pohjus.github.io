...
void main() throws Exception {
    ...
    // Test 1: source contains returnManyTimes method with char and int parameters
    total++;
    if (source.matches("(?s).*String\\s+returnManyTimes\\s*\\(\\s*char\\s+\\w+\\s*,\\s*int\\s+\\w+\\s*\\).*")) {
        IO.println("  PASS: source contains returnManyTimes(char, int) method");
        passed++;
    } else {
        IO.println("  FAIL: expected a 'String returnManyTimes(char, int)' method in Main.java");
    }
